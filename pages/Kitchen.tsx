...
export const Kitchen = () => {
  ...
  const activeOrders = useMemo(() => {
    ...
    const isAdmin =
      currentUser &&
      [Role.OWNER, Role.MANAGER, Role.SUPER_ADMIN].includes(currentUser.role);
    ...
    const hasAssignedCats =
      isKitchen &&
      currentUser?.assignedCategories &&
      currentUser.assignedCategories.length > 0;
    ...
    if (hasAssignedCats) {
      filtered = filtered.filter((order) => {
        ...
        return order.items.some((item) => {
          ...
          return (
            menuItem &&
            currentUser.assignedCategories?.includes(menuItem.category)
          );
        });
      });
    }
  ...
  const handleSystemPrint = async (order: any, type: "kot" | "invoice") => {
    ...
    const tokenEl =
      tempDiv.querySelector(".kot-token") ||
      tempDiv.querySelector(".invoice-token");
    if (tokenEl)
      tokenEl.textContent = `${type === "kot" ? "Kitchen Token" : "Invoice"}: #${order.tokenNumber}`;

    const tableEl =
      tempDiv.querySelector(".kot-table") ||
      tempDiv.querySelector(".invoice-table");
    if (tableEl)
      tableEl.textContent = `Table No: ${order.tableNumber || "Delivery"}`;

    const creatorName = order.creatorName || getCreatorName(order.createdBy);
    const waiterEl =
      tempDiv.querySelector(".kot-waiter") ||
      tempDiv.querySelector(".invoice-waiter");
    if (waiterEl) waiterEl.textContent = `Ordered by: ${creatorName}`;

    const itemsContainer =
      tempDiv.querySelector(".kot-items-container") ||
      tempDiv.querySelector(".invoice-items-container");
    if (itemsContainer) {
      ...
      grouped.forEach((item: any) => {
        ...
        itemDiv.className =
          "flex justify-between items-start text-[10pt] font-black border-b border-dashed border-black pb-1 mb-1";
        if (type === "kot") {
      ...
      if (type === "invoice") {
        ...
        totalDiv.className =
          "flex justify-between items-center text-[12pt] font-black border-t-2 border-black pt-2 mt-2";
    ...
    if (type === "kot") {
      const noteContainer = tempDiv.querySelector(
        ".kot-note-container",
      ) as HTMLElement;
      if (noteContainer) {
        if (order.note) {
    ...
    if (
      type === "kot" &&
      currentTenant?.printerSettings?.autoMarkReadyOnPrint
    ) {
  ...
  const canMoveToReady = (userRole: Role) => {
    return [Role.OWNER, Role.MANAGER, Role.WAITER, Role.KITCHEN].includes(
      userRole,
    );
  };
    ...
    <div className="p-4 md:p-10 h-full overflow-y-auto bg-slate-50/50 no-scrollbar">
      <div className="mb-8 md:mb-12 flex flex-col md:flex-row md:items-center gap-8">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-fit">
            <h1 className="text-2xl font-black text-[#1a1a37] uppercase tracking-tighter">
              Live Kitchen
            </h1>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="bg-rose-100 px-6 py-3 rounded-2xl border-2 border-rose-200 shadow-sm text-[10px] font-black uppercase tracking-widest text-rose-800 w-full md:w-72 flex items-center justify-between">
            <span className="min-w-[120px]">
              {filter === "pending" ? "Active Orders" : "Completed Orders"}
            </span>
        ...
        {activeOrders.length === 0 ? (
          ...
          paginatedOrders.map((order) => {
            ...
            const hasPreparing = order.items.some(
              (i) => i.status === OrderStatus.PREPARING,
            );
            ...
            return (
              <div
                key={order.id}
                className="group relative rounded-[2.5rem] shadow-xl bg-white flex flex-col h-full transition-all border-4 border-black min-h-[400px] overflow-hidden"
              >
                {/* Top Border Color */}
                <div
                  className={`absolute top-0 left-0 right-0 h-4 ${statusColors.bg}`}
                ></div>
                ...
                <div className="relative z-10 flex flex-col h-full p-6 pt-10">
                  <div className="flex flex-col items-center mb-4">
                    ...
                    <div
                      className={`w-10 h-1.5 rounded-full mt-1.5 ${statusColors.bg}`}
                    ></div>
                  </div>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (currentTenant?.printerSettings?.enablePrintAgent) {
                          ...
                          const settings = currentUser?.printerSettings
                            ?.pairedPrinterId
                            ...
                            try {
                              ...
                              if (result.success) {
                                if (isDone) {
                                  ...
                                  await BluetoothPrinterService.printKOT(
                                    currentTenant!,
                                    orderData as any,
                                    settings,
                                  );
                  ...
                  {order.orderType === "Online Delivery" &&
                    order.status === OrderStatus.READY && (
                      <div className="mb-4">
                        <select
                          ...
                          onChange={(e) => {
                            const selectedStaff = deliveryStaff.find(
                    ...
                    {(() => {
                      ...
                      order.items.forEach((item) => {
                        ...
                        if (
                          ...
                          if (
                            !menuItem ||
                            !currentUser.assignedCategories.includes(
                              menuItem.category,
                            )
                          ) {
                      ...
                      return Object.entries(groupedItems).map(
                        ([key, group], index) => {
                          const itemStatusColors = getStatusColors(
                            group.status,
                          );
                          return (
                            <div
                            ...
                            >
                              ...
                              <div className="relative">
                                ...
                                <ChevronDown
                                  className={`absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none ${itemStatusColors.text}`}
                                  size={12}
                                />
                              </div>
                            </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-900">
                          {getCreatorName(order.createdBy).split(" ")[0]}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (
                              currentTenant?.printerSettings?.enablePrintAgent
                            ) {
                              try {
                                await createPrintRequest(
                                  orderData as any,
                                ...
                                console.error("Print Agent failed:", error);
                                alert(
                                  "Cloud print failed. Falling back to system print.",
                                );
                                await handleSystemPrint(
                                  orderData as any,
                                  "invoice",
                            ...
                            if (settings?.pairedPrinterId) {
                              try {
                                ...
                                console.error("Bluetooth print failed:", error);
                                alert(
                                  "Bluetooth print failed. Falling back to system print.",
                                );
                                await handleSystemPrint(
                                  orderData as any,
                                  "invoice",
                              ...
                              alert(
                                "No printer configured. Fallback to PC printer.",
                              );
                              await handleSystemPrint(
                                orderData as any,
                                "invoice",
                ...
                {filteredMenu.map((item) => (
                  ...
                  >
                    <div className="text-left">
                      <p className="text-sm font-black text-slate-900 capitalize tracking-tight group-hover:text-indigo-600">
                        {item.name}
                      </p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        {item.category}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center border-2 border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
                        ...
                        <div className="w-10 h-8 flex items-center justify-center font-black text-sm text-slate-900 bg-slate-50/50">
                          {item.quantity}
                        </div>
                        <button
                          onClick={() => {
      ...
      <div id="kot-content" className="hidden p-4 font-sans text-black">
        <div className="text-center mb-4 border-b-2 border-black pb-2">
          <h2 className="font-bold text-[16pt] uppercase kot-token">
            Kitchen Token: #0
          </h2>
        ...
        <div className="kot-note-container hidden border-t border-black pt-2 italic">
          <p className="text-[10pt]">
            Note: <span className="kot-note"></span>
          </p>
        </div>
